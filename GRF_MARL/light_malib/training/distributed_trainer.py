...
from light_malib.utils.device import resolve_device
...
class DistributedPolicyWrapper:
    ...
    def __init__(self, policy, local_rank, device, use_ddp):
        Logger.info(
            "local_rank: {} cuda_visible_devices:{}".format(
                local_rank, os.environ.get("CUDA_VISIBLE_DEVICES", "N/A")
            )
        )
        self.device = device
        self.policy = policy.to_device(self.device)
        self._use_ddp = use_ddp and self.device.type == "cuda"
    ...
    def _wrap(self, key, distributed=True):
        if hasattr(self.policy,key):
            ...
            if isinstance(value,nn.Module) and len(list(value.parameters()))>0:
                ...
                if distributed and self._use_ddp:
                    device_ids = [0]
                    setattr(self.policy,key,DistributedDataParallel(getattr(self._policy,key), device_ids=device_ids, find_unused_parameters=False))
                else:
...
class DistributedTrainer:
    ...
    def reset(self, training_desc: TrainingDesc):
        ...
        resolved_device = resolve_device(
            self.cfg.get("device"),
            self.cfg.get("use_cuda", False),
        )
        self.device = resolved_device
        use_ddp = self.cfg.get(
            "use_ddp", self.world_size > 1 and resolved_device.type == "cuda"
        )
        self.policy = DistributedPolicyWrapper(
            policy_desc.policy, self.local_rank, resolved_device, use_ddp
        )
