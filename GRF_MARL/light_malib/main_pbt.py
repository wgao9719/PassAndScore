...
def main():
    ...
    # check cfg
    # check gpu number here (allow cpu-only setups)
    cluster_gpus = ray.cluster_resources().get("GPU", 0)
    trainer_gpu = cfg.training_manager.trainer.distributed.resources.get("num_gpus", 0)
    required_gpu = trainer_gpu * cfg.training_manager.num_trainers
    assert (
        required_gpu <= cluster_gpus
    ), "#trainer gpus({}) should be <= #cluster gpus({})".format(
        required_gpu, cluster_gpus
    )
